...
function addReportMarker(r) {
  ...
  if (r.images && r.images.length) {
    imgsHtml =
      '<div class="map-popup-imgs">' +
      r.images
        .slice(0, 2)
        .map((u) => `<img src="${u}" alt="Report image" loading="lazy" />`)
        .join("") +
      "</div>";
  }
...
function initReportModal() {
    ...
    useLocBtn.addEventListener("click", () => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          if (err.code === 1)
            msg =
              "Location permission denied. Please allow access or set location manually.";
          if (err.code === 2)
            msg = "Location unavailable. Try manual entry instead.";
  ...
  function openModal() {
    ...
    if (window._pickedLat !== undefined) {
      document.getElementById("lat-display").value =
        window._pickedLat.toFixed(6);
      document.getElementById("lng-display").value =
        window._pickedLng.toFixed(6);
    }
  }
  function closeModal() {
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    if (lat == null || isNaN(lat) || lng == null || isNaN(lng))
      return showErr(
        "Please set a location — click the map or enter coordinates manually.",
      );
    if (lat < -90 || lat > 90)
      return showErr("Latitude must be between -90 and 90.");
    if (lng < -180 || lng > 180)
      return showErr("Longitude must be between -180 and 180.");
      ...
      if (data.ok) {
        ...
        if (data.invalid) {
          ...
          document.querySelector(
            "#submit-success div:last-of-type",
          ).textContent =
            data.ai_note ||
            "This report doesn't appear to show a valid street issue and won't be reviewed by admins.";
        } else {
          ...
          document.querySelector(
            "#submit-success div:last-of-type",
          ).textContent = "Your report is now pending admin review.";
        }
        resetModal();
      } else {
  ...
  function resetModal() {
    ...
    document.querySelectorAll(".img-input").forEach((i) => {
      i.value = "";
    });
    document.querySelectorAll(".img-preview").forEach((p) => {
      p.style.display = "none";
      p.src = "";
    });
    document.querySelectorAll(".img-slot-inner").forEach((i) => {
      i.style.display = "flex";
    });
    document
      .querySelectorAll(".img-slot")
      .forEach((s) => s.classList.remove("has-img"));
    document.getElementById("lat-display").value = "";
    document.getElementById("lng-display").value = "";
